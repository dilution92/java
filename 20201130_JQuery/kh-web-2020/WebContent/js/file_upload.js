...
function append(zone){
	...
	file.setAttribute("name", "attfile" + fileAttCnt); //반드시 name이 존재해야 한다.
	file.setAttribute("style", "display: none;");
	file.setAttribute("modify", "no");  // file의 선택 여부를 체크
	...
	delBtn.onclick = function(ev){
		...
		var file = p.getElementsByClassName("attfile").itme[0];
...
function imagePreView(zone, ev){
   ...
   file.click();// 이미지를 클릭하면 파일태그를 클릭한 효과를 준다구   
