	...
	<fieldset>
		...
		<ul>
			...
			<li>명령어는 filter와 traverse가 매우 유사하거나 상충되기 때문에
				개인적 취향에 따라 사용 빈도ㅜ가 달라질 수 있다.</li>
			<li>종류 => .eq(index) 해당인덱스,
				.first 처음, 
				.last 마지막, 
				.slice(start [, end])
			</li>
			<li>method => .children(expr), .find(expr), .parent(expr), .next(expr), .prev(expr)
...
<script type="text/javascript">
	//첫번째 글자 색, 필터랑 같이 쓰기 글자 색, 범위 지정 궁서체로
	$('#traverse>ul>li').first().css('color','#f00');
	$('#traverse>ul>li').filter(':odd').css('color', '#00f');
	$('#traverse>ul>ul').slice(1,4).css('font-family','궁서체');
